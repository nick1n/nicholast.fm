...
function LastFM(options) {
	...
	var apiKey		= options.apiKey	|| 'f750712ed70caea3272e70e48e1f464e';
	...
	var internalCall = function(params, callbacks, requestMethod) {
		...
		if(requestMethod == 'POST') {
			...
			for (var param in params) {
				array.push(param + "=" + encodeURIComponent(params[param])); //.toString().replace(/&/g, encodeURIComponent("&")).replace(/\+/g," ").replace(/</g," ").replace(/>/g," ").replace(/\?/g," ").replace(/=/g," ")
			}
			array.push('callback=?');
			
			var xhr = $.getJSON(apiUrl + '?' + array.join('&'), function(data) {
				console.log("success");
				console.log(data);
		...
		/* Cross-domain GET request (JSONP). */
			...
			// My added code for calls that are sent out at the exact millisecond as others - Nicholas Ness
			...
			// check to see if resources were loaded....?
			//scripts[jsonp] = script;
			//setTimeout("getRecentTracks('" + jsonp + "')", 200*page);
		}
	};
